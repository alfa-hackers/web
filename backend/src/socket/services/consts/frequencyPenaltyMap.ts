export const frequencyPenaltyMap: Record<string, number> = {
  text: process.env.MODEL_FREQUENCY_PENALTY_TEXT
    ? parseFloat(process.env.MODEL_FREQUENCY_PENALTY_TEXT)
    : 0,
  pdf: process.env.MODEL_FREQUENCY_PENALTY_PDF
    ? parseFloat(process.env.MODEL_FREQUENCY_PENALTY_PDF)
    : 0,
  word: process.env.MODEL_FREQUENCY_PENALTY_WORD
    ? parseFloat(process.env.MODEL_FREQUENCY_PENALTY_WORD)
    : 0,
  excel: process.env.MODEL_FREQUENCY_PENALTY_EXCEL
    ? parseFloat(process.env.MODEL_FREQUENCY_PENALTY_EXCEL)
    : 0,
